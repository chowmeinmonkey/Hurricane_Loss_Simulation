...
def simulate_storm():
    # FIX: Corrected unmatched parenthesis
...
        # Approximate distance calculation (not precise, but functional for model)
...
def quick_sim(hpy, wm, ws, cf): # Pass parameters to cache correctly
    losses = []
    for _ in range(300):
...
    Example: If the curve passes through (10 000 000, 0.01) → there is a <strong>1-in-100-year chance</strong> of losses > $10 million.<br>
...
    st.markdown("""
    <div class="explanation">
    • <strong>Red dot</strong> = hurricane eye<br>
    • <strong>Translucent red circle</strong> = full radius of hurricane-force winds (∼ wind speed × 0.5 km)<br>
    • Damage occurs anywhere inside this circle — that’s why even storms that “miss” Florida can still cause losses.
    </div>
    """, unsafe_allow_html=True)

    # Use session_state to control button behavior and persistence
    if st.button("Launch Storm", key="launch_storm_button"):
        # Store the generated storm data
    ...
    if st.session_state.storm_launched and st.session_state.storm_data:
        ...
        # Base time for animation timestamps
        ...
        for h in range(16):
            ...
            # The exact time for this step
            current_time = base_time + pd.Timedelta(hours=h)
            time_str = current_time.isoformat()

            # 1. The moving radius circle (FIX: Use properties and proper scaling)
            # The radius parameter in the style object is relative to the map scale
            features.append({
                "type": "Feature",
                "geometry": {"type": "Point", "coordinates": [lon, lat]},
                "properties": {
                    "time": time_str, # Required for TimestampedGeoJson
                    "popup": f"Radius – {wind_now:.0f} mph",
                    "style": {
                        # Scaling radius relative to wind for visual effect on map
            ...
            # 2. The eye marker (FIX: Use properties and proper icon style)
            features.append({
                "type": "Feature",
                "geometry": {"type": "Point", "coordinates": [lon, lat]},
                "properties": {
...
    st.markdown("""
    <div class="explanation">
    • Red circle = full hurricane-force wind field<br>
    • Heat = damage × insured value at each city
    </div>
    """, unsafe_allow_html=True)
...
        CircleMarker(
            location=center,
            # radius is now proportional to the visual scaling factor on the map
            radius=wind*900, 
            color="#ef4444",
            weight=3,
            fillOpacity=0.18,
