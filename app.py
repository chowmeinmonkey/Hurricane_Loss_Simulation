...
with st.sidebar:
    ...
    wind_mean = st.slider("Mean max wind (mph)", 80, 180, 110, 5)
    wind_std = st.slider("Wind std dev (mph)", 10, 50, 25, 5)
...
with tab2:
    ...
    if st.button("Launch Storm"):
        ...
        for h in range(16):
            ...
            radius_meters = wind_now * 500  # visual scaling

            # The moving radius circle
            features.append({
                "type": "Feature",
                "geometry": {"type": "Point", "coordinates": [lon, lat]},
                "properties": f"2025-09-01T{h:02d}:00:00",
                "style": {"radius": radius_meters/70, "fillColor": "#ef44440", "color": "#ef4444", "weight": 2, "opacity": 0.8, "fillOpacity": 0.15}
            })
            # The eye marker
            features.append({
                "type": "Feature",
        ...
        TimestampedGeoJson(
            ...
            period="PT1H", auto_play=True, loop=False, add_last_point=True
        ).add_to(m)
...
with st.expander("Technical Methodology"):
    ...
    - Intensity: Normal(μ × climate⁰·⁴, σ), ≥74 mph  
    - Footprint: Circular radius ≈ wind × 0.5 km  
